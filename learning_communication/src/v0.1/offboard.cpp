...
//地坐标系转为全局坐标系
int coordinate(int pos, int dist)
{
    return pos-dist;
}
...
int main(int argc,char **argv)
{
    ...
    int index = *argv[1] - 48;
    int dist_x = *argv[2] - 48;
    int dist_y = *argv[3] - 48;
...
//    nh.param<int>("uav_name", index, 0);
    ...
    double track [10][2];
    track[0][0] = -6.5;track[0][1] = 4;
    track[1][0] = 6.5;track[1][1] = 4;
    track[2][0] = -6.5;track[2][1] = 9.3;
    track[3][0] = 6.5;track[3][1] = 9.3;
    track[4][0] = -6.5;track[4][1] = 14.2;
    track[5][0] = 6.5;track[5][1] = 14.2;
    track[6][0] = -6.5;track[6][1] = 19.6;
    track[7][0] = 6.5;track[7][1] = 19.7;
    track[8][0] = -6.5;track[8][1] = 25;
    track[9][0] = 6.5;track[9][1] = 25;
    ...
    ros::Subscriber position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/uav0/mavros/local_position/pose",10,position_cb);
    ros::Subscriber target_pos = nh.subscribe<std_msgs::Float64MultiArray>("/track", 10, target);

//    int index_1 = list_1.data[index];
    target_x = track[index][0];
    target_y = track[index][1];
...
//    ROS_INFO("command pos [%f],[%f],[%d],[%s]", target_x,target_y,index,argv[1]);
    ...
    while (ros::ok()){
        ...
        if (signal == 1)
        {
            ...
            if (timing == 1){
                pose.pose.position.x = coordinate(0, dist_x);
                pose.pose.position.y = coordinate(0, dist_y);
                pose.pose.position.z = index*1+0.5;
                local_pos_pub.publish(pose);
            }

            if (timing == 2){
                pose.pose.position.x = coordinate(0, dist_x);
                pose.pose.position.y = coordinate(target_y, dist_y);
                pose.pose.position.z = 1.5;
                local_pos_pub.publish(pose);
            }

            if (timing == 3){
                pose.pose.position.x = coordinate(target_x, dist_x);
                pose.pose.position.y = coordinate(target_y, dist_y);
                pose.pose.position.z = 1.5;
                local_pos_pub.publish(pose);
            }
            ...
            if (timing == 5){
                pose.pose.position.x = coordinate(0, dist_x);
                pose.pose.position.y = coordinate(25, dist_y);
                pose.pose.position.z = 0.5*index+0.5;
                local_pos_pub.publish(pose);
            }
            if (timing == 6){
                pose.pose.position.x = coordinate(6.5-index*0.5, dist_x);
                pose.pose.position.y = coordinate(25, dist_y);
                pose.pose.position.z = 1.5;
                local_pos_pub.publish(pose);
            }
...
//            ROS_INFO("---commandddddd pos [%f],[%f],[%d]", target_x,target_y,timing);
        ...
        cout<<*argv[1]<<"--------111"<<*argv[2]<<"1111111111111"<<*argv[3]<<endl;
