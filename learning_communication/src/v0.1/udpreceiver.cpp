...
// udp para
    ...
    while (ros::ok()){
        ...
        switch(select(sock,&fds,&fds,NULL,&timeout)) //select使用
        {
...
//        && current_state.mode != "AU.LOITER"
        if (current_state.mode != "AUTO.TAKEOFF" && (smsg[3]==1) && current_state.position[2]<target_height){
            if (set_mode_client.call(takeoff_set_mode)){
                ROS_INFO("takeoff enabled");
            }
        }
        if (current_state.mode!="OFFBOARD" && (smsg[3]==2) && ros::Time::now()-last_request>ros::Duration(5.0)){
            if(set_mode_client.call(offb_set_mode) ) {
                ROS_INFO("offboard enable");
            }
            last_request = ros::Time::now();
        }else if (!current_state.armed && (smsg[3]!=0) && ros::Time::now()-last_request>ros::Duration(5.0)){
